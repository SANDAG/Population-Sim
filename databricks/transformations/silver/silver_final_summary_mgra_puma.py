...
@dp.table(
    comment="Cleaned PUMA-level final summary with validated geography and id",
    table_properties={"delta.feature.timestampNtz": "supported"},
)
@dp.expect_or_drop("valid_id", "id IS NOT NULL")
def silver_final_summary_mgra_puma():
    lookup = spark.read.table("run_id_lookup")
    return (
        spark.readStream.table("bronze_final_summary_mgra_puma")
        .drop("_rescued_data")
