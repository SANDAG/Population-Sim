...
def load_all_data_for_year(staging_table, year, conn):
    data = {
        "sex": load_demographic_data(
            "../sql/mgra_control_sex.sql", staging_table, year, conn
        ),
        "age": load_demographic_data(
            "../sql/mgra_control_age.sql", staging_table, year, conn
        ),
        "race": load_demographic_data(
            "../sql/mgra_control_ethnicity.sql", staging_table, year, conn
        ),
        "household": load_demographic_data(
            "../sql/mgra_control_hh_char.sql", staging_table, year, conn
        ),
        "income": load_demographic_data(
            "../sql/mgra_control_income.sql", staging_table, year, conn
        ),
    }
    return data
...
def manipulate_race_data(df):
    ...
    df_pivot.columns.name = ""
    df_pivot["Non-Hispanic, Other"] = (
        df_pivot["Non-Hispanic, Other"]
        + df_pivot["Non-Hispanic, American Indian or Alaska Native"]
        + df_pivot["Non-Hispanic, Hawaiian or Pacific Islander"]
    )
...
def combine_dataframes(dataframes_dict):
    ...
    combined_df = reduce(
        lambda left, right: pd.merge(left, right, on="mgra"), dataframes_to_merge
    )

    # Calculate additional columns if needed
    combined_df["Total_HH_GQ"] = (
        combined_df["Total_HH"]
        + combined_df["gq_college_pop"]
        + combined_df["gq_mil_pop"]
        + combined_df["gq_other_pop"]
    )
    return combined_df
